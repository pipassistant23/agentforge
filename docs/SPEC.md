...
│  │    • mcp__agentforge__* (scheduler tools via IPC)                │   │
│  │                                                                │   │
│  └──────────────────────────────────────────────────────────────┘   │
│                                                                      │
└──────────────────────────────────────────────────────────────────────┘
...
│   ├── pipbot.log               # Host stdout
│   └── pipbot.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.agentforge.plist         # macOS service configuration
```
...
Pipbot uses a hierarchical memory system based on CLAUDE.md files.
...
[Jan 31 2:35 PM] John: @YourBot what toppings do you recommend?
```
...
AgentForge has a built-in scheduler that runs tasks as full agents in their group's context.
...
```
User: @YourBot remind me every Monday at 9am to review the weekly metrics

Claude: [calls mcp__agentforge__schedule_task]
        {
...
```
User: @YourBot at 5pm today, send me a summary of today's emails
...
### AgentForge MCP (built-in)

The `agentforge` MCP server is created dynamically per agent call with the current group's context.
...
AgentForge runs as a single macOS launchd service.
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/agentforge.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/agentforge.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.agentforge.plist ~/Library/LaunchAgents/

# Start service
launchctl load ~/Library/LaunchAgents/com.agentforge.plist

# Stop service
launchctl unload ~/Library/LaunchAgents/com.agentforge.plist
...
- `logs/agentforge.log` - stdout
- `logs/agentforge.error.log` - stderr
