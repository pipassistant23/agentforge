  ...
  for await (const message of query({
    ...
    options: {
      ...
      mcpServers: {
        ...
        qmd: {
          command: path.join(process.cwd(), 'node_modules', '@tobilu', 'qmd', 'qmd'),
          args: ['mcp'],
    ...
    if (!memoryFlushTriggered && messageCount >= MEMORY_FLUSH_MESSAGE_THRESHOLD) {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    log(`QMD initialization failed (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
  }
